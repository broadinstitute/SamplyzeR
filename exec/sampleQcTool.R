...
createSds <- function (bamQcInput, vcfQcInput, annotInput, stratify, primaryID) {
  stratify = unlist(strsplit(stratify, ',')
  sds = sampleDataset(annotInput, primaryID, bamQcInput, vcfQcInput, stratify)
}
...
#ggplot2::ggsave(paste(opt$prefix, stratify,'QcPlot.pdf', sep = '.'), grobList,
#  width = 12, height = 15)
}
...
option_list <- list(
  make_option(c("-b", "--bamQcTsv"), default = NULL,
              help = "A tsv for BAM Level quality control metrics"),
  make_option(c("--QcCutoffs", default = NULL,
                help = 'a tsv file that contains hard cutoffs for QC metrics')),
  make_option(c("-v", "--vcfQcTsv"), default = NULL,
              help = "A tsv for VCF Level quality control metrics"),
  make_option(c("-a", "--annotTsv",
                help = "A tsv for sample annotation file")),
  make_option(c("-d", "--primaryID"),
              help = "primary IDs used to specific each sample"),
  make_option(c("-s", "--stratify"),
              help = "A list of attributes used in the stratified QC metrics
              analysis, separated by comma"),
  make_option(c("-g", "--samplePc"), default = NULL,
              help = 'A tsv file with genotype PCs for each sample'),
  make_option(c("-r", "--refPc"), default = NULL,
              help = 'A tsv file with genotype PCs from a reference set of
              samples'),
